...
public abstract class BaseController {
	...
	public List<Cart> getAllProductFromSession(int productId,int productNum, HttpServletRequest request){
		logger.debug("addProductToSession");
	 ...
	 * @return
	 */
	public double getCartPrice(List<Cart> cartList,Customer customer){
		Double totalPrice =  cartList.stream().map(cart -> cart.getProduct().getPrice1()*cart.getProductNum()).reduce(0D, (num1,num2)->(num1+num2));
		if(customer!=null){
			totalPrice*=customer.getDiscount();
		}
		NumberFormat nf = new DecimalFormat("0.00");
		return new Double(nf.format(totalPrice));
	}
}
