package com.easycart.controller.web.cart.logic;
...
public class CartLogic extends BaseLogic {
	 ...
	 */
	public Double[] getProductTotalPrice(List<ProductView> productViewList, double discount,ToArea toArea) {
		...
		if (productViewList.size() > 0) {
			...
			total[1] = new Double(nf.format(totalWeight * (lowestUnitPriceMenu.getUnitePrice() + toArea.getPriceAdjust())));
		}
		return total;
	}
