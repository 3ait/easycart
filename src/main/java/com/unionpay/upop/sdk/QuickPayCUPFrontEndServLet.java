...
public class QuickPayCUPFrontEndServLet extends HttpServlet {
	/**
	 * 
	 */
...
//                        return;
		...
		String[] resArr = new String[QuickPayConf.reqVo.length];
		for(int i=0;i<QuickPayConf.reqVo.length;i++){
			resArr[i] = request.getParameter(QuickPayConf.reqVo[i]);
                        map.put(QuickPayConf.reqVo[i], resArr[i]);
		}
		...
		try {
			Boolean signatureCheck = new QuickPayUtils().checkSign(QuickPayConf.reqVo, resArr, signMethod, signature);
                        ...
                        if(signatureCheck){
                            ...
                            for (int i = 0; i < QuickPayConf.notifyVo.length; i++) {
                                    returnMap.put(QuickPayConf.notifyVo[i], map.get(QuickPayConf.notifyVo[i]) == null ? "" : map.get(QuickPayConf.notifyVo[i]));
                            }
