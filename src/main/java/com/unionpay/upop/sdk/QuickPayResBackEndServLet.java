...
public class QuickPayResBackEndServLet extends HttpServlet {
	/**
	 * 
	 */
	...
	public void service(HttpServletRequest request, HttpServletResponse response) {
		...
		response.setCharacterEncoding(QuickPayConf.charset);
			...
			Boolean signatureCheck = new QuickPayUtils().checkSign(QuickPayConf.notifyVo, resArr, signMethod, signature);
                        System.out.println("签名是否正确："+ signatureCheck);
			System.out.println("交易是否成功："+"00".equals(map.get("respCode")));
			if(!"00".equals(resArr[10])){
                            System.out.println("响应码:" + map.get("respCode"));
                            System.out.println("失败原因:" + map.get("respMsg"));
			}
